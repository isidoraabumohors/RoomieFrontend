...
function MyMatches() {
  ...
  const fetchMatchesData = async () => {
    try {
      ...
      const matchesResponse = await axios.get(`${API_URL}/matches/profile/${profileid}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      const processedMatches = await Promise.all(
        matchesData.map(async (match) => {
          ...
          const profileResponse = await axios.get(`${API_URL}/profiles/${otherProfileId}`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
  ...
  const handleChatButtonClick = async (matchId) => {
    try {
      ...
      console.log(matchId);
      const chatCheckResponse = await axios.get(`${API_URL}/chats/match/${matchId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
