...
function Chat() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const messagesResponse = await axios.get(`${API_URL}/messages/chat/${chatid}`, {
          headers: {
    ...
    const fetchOtherUserProfile = async () => {
      try {
        ...
        const otherProfileResponse = await axios.get(`${API_URL}/profiles/${otherProfileId}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const handleSendMessage = async () => {
    if (newMessage.trim() !== '') {
      try {
        ...
        const sendMessageResponse = await axios.post(`${API_URL}/messages`, messageData, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
