...
function ViewMatch() {
  ...
  const fetchData = async () => {
    try {
      const responseUserIdA = await axios.get(`${API_URL}/profiles/${profile_A_id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const userIdA = responseUserIdA.data.user_id;

      const responseA = await axios.get(`${API_URL}/users/${userIdA}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const responseUserIdB = await axios.get(`${API_URL}/profiles/${profile_B_id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const userIdB = responseUserIdB.data.user_id;

      const responseB = await axios.get(`${API_URL}/users/${userIdB}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setProfileB(responseB.data);

      const responseMatch = await axios.get(`${API_URL}/chats/matches/${matchId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const chatId = responseMatch.data.id;
      setChatId(chatId); 

      const responseLocation = await axios.get(`${API_URL}/locations/${locationId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const responseMessages = await axios.get(`${API_URL}/messages/chat/${chatId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteAllMessages = async () => {
    try {
      ...
      await axios.delete(`${API_URL}/messages/chat/${chatId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchData(); 
    } catch (error) {
